...
from StressEncoder import StressFormer
...
class SpatialInput(nn.Module):
    ...
    def forward(self, scalar, categorical, gating, new_scaler):

        shape = scalar.shape
        shape2 = categorical.shape
        scalar = scalar / new_scaler
...
class RelationalCore(nn.Module):
    def __init__(self):
        ...
        self.transform2 = StressFormer(128, 4, 512, dropout=0.0)
        self.transform3 = nn.TransformerEncoderLayer(128, 4, 512, dropout=0.0)

    def forward(self, screen, minimap, state, tot_result):
        shape = screen.shape
        ...
        #x = self.transform2(x)
        x = self.transform2(x, tot_result)
        x = self.transform3(x)
        x = x.transpose(0, 1)
...
class Model(nn.Module):
    ...
    def forward(self, inputs, hidden, tot_result=None, targets=None):
        ...
        feature_screen = torch.tensor(values, device='cuda:0')
        values = [[[[[255]]]]]
        minimap_screen = torch.tensor(values, device='cuda:0')

        screen_input, screen_bypass = self.screen_input(
            inputs["feature_screen_scalar"].float(),
            inputs["feature_screen_categorical"].long(), scalar_input, feature_screen)
        minimap_input, minimap_bypass = self.minimap_input(
            inputs["feature_minimap_scalar"].float(),
            inputs["feature_minimap_categorical"].long(), scalar_input, minimap_screen)
        hidden_1 = hidden[:2]
        hidden_2 = hidden[2:]

        screen_output, minimap_output, lstm_input, next_hidden1 = self.core(
            screen_input, minimap_input, hidden_1, tot_result)
