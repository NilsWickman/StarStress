...
def launch_games2(model,
    ...
    with torch.no_grad():
        ...
        for games, workers, opponent, group_name in test_groups:
            ...
            draw = group_name + "_draw"
            total = group_name + "_total"
            print(draw)
            print(type(draw))
            rewards[draw] = 0
            rewards[total] = 0
        ...
        while True:
            ...
            while len(envs) < min(max_batch, envs_left):
                ...
                if reward > 0:
                    rewards[g.group_name + "_total"] += 1
                    rewards[g.group_name] += reward
                    print(rewards) 
                elif reward == 0:
                    rewards[g.group_name + "_total"] += 1
                    rewards[g.group_name + "_draw"] += 1
                    print(rewards)  
                elif reward == -1:
                    rewards[g.group_name + "_total"] += 1
                    print(rewards)
...
class Evaluator:
    ...
    def try_get_results(self, block=0):
...
def evaluate(model_path, reference_replay, race):
    ...
    #test_groups = [[100, 2, Bot(race, Difficulty.very_easy), "very_easy"],
    #               [0, 2, Bot(race, Difficulty.easy), "easy"],
    #               [0, 2, Bot(race, Difficulty.medium), "medium"],
    #               [0, 2, Bot(race, Difficulty.hard), "hard"]]
    test_groups = [[100, 12, Bot(race, Difficulty.very_easy), "very_easy"]]
    ev = Evaluator(reference_replay)
    ev.start_eval(model, 0, test_groups)
    print(ev.try_get_results(block=None))
    return(ev.try_get_results(block=None))
...
if __name__ == "__main__":
    ...
    #run_lan_game()
    if args.mode == 'BATCH':
        x = evaluate(args.model_path, args.reference_replay, race)
        print(x)
    elif args.mode == 'LAN':
        host = "127.0.0.1"
